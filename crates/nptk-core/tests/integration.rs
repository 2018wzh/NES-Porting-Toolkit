...
fn test_cpu_reset_jumps_to_vector() {
    ...
    assert_eq!(sys.cpu.registers.program_counter, 0x8000);
}
...
fn test_cpu_ram_read_write() {
    ...
    sys.step_cpu();
    assert_eq!(sys.cpu.registers.accumulator, 0x42);
...
fn test_cpu_flag_behavior() {
    ...
    let mut sys = make_system(prg);
    sys.step_cpu();
    assert!(sys.cpu.registers.status.contains(mos6502::registers::Status::PS_ZERO));
    assert!(!sys.cpu.registers.status.contains(mos6502::registers::Status::PS_NEGATIVE));
}
...
fn test_ppu_vblank_sets_nmi() {
    ...
    sys.run_frame();
    // mos6502 内部通过 Bus::nmi_pending() 自动处理 NMI，不再需要外部检查
}
...
fn test_apu_registers_accessible() {
    ...
    sys.cpu.memory.cpu_write(0x4000, 0x9F); // Pulse 1 duty/volume
    sys.cpu.memory.cpu_write(0x4001, 0x00); // Pulse 1 sweep
    sys.cpu.memory.cpu_write(0x4002, 0x42); // Pulse 1 timer low
    sys.cpu.memory.cpu_write(0x4003, 0x00); // Pulse 1 length/timer high
    // Read back through bus
    assert!(sys.cpu.memory.cpu_read(0x4000) != 0xFF, "APU should respond");
}
...
fn test_controller_strobe_and_read() {
    ...
    state.a = true;
    sys.cpu.memory.controller[0].set_current(state);

    // Strobe: latch the state
    sys.cpu.memory.controller[0].write_strobe(1);
    sys.cpu.memory.controller[0].write_strobe(0);

    // First read should return A (bit 0)
    let first = sys.cpu.memory.controller[0].read();
