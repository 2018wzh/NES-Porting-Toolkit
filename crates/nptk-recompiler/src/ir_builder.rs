...
use crate::ir6502::{BranchCondition, IrBlock, IrOp, Label, Operand};
...
impl IrBuilder {
    ...
    pub fn lift_block(bytes: &[u8], start_addr: u16) -> Vec<IrOp> {
        let mut ops = Vec::new();
        let mut offset = 0;
    ...
    fn instruction_cycles(opcode: u8) -> u8 {
        match opcode {
            ...
            | 0xB8 | 0xC8 | 0xD8 | 0xE8 | 0xF8 => 2,
            0x0A | 0x2A | 0x4A | 0x6A | 0x8A | 0x9A | 0xAA | 0xBA | 0xCA | 0xEA => 2,
            0x40 | 0x60 => 6, // RTI, RTS
            // 2-byte immediate
            0xA9 | 0xA2 | 0xA0 | 0x69 | 0xE9 | 0xC9 | 0xE0 | 0xC0 | 0x29 | 0x09 | 0x49 => 2,
            // 2-byte zeropage
            0xA5 | 0x85 | 0xA6 | 0x86 | 0xA4 | 0x84 | 0x65 | 0xE5 | 0x25 | 0x05 | 0x45 | 0xC5
            | 0xE4 | 0xC4 | 0xE6 | 0xC6 | 0x24 | 0x06 | 0x46 | 0x26 | 0x66 => 3,
            0xB5 | 0x95 | 0xB4 | 0x94 | 0x75 | 0xF5 | 0x35 | 0x15 | 0x55 | 0xD5 | 0xF6 | 0xD6
            | 0x16 | 0x56 | 0x36 | 0x76 => 4,
            0xB6 | 0x96 => 4,
            // indirect
            0xA1 | 0x81 | 0x61 | 0xE1 | 0x21 | 0x01 | 0x41 | 0xC1 => 6,
            0xB1 | 0x91 | 0x71 | 0xF1 | 0x31 | 0x11 | 0x51 | 0xD1 => 5,
            // 3-byte absolute
            0xAD | 0x8D | 0xAE | 0x8E | 0xAC | 0x8C | 0x6D | 0xED | 0x2D | 0x0D | 0x4D | 0xCD
            | 0xEC | 0xCC | 0xEE | 0xCE | 0x2C | 0x0E | 0x4E | 0x2E | 0x6E => 4,
            0xBD | 0x9D | 0xBC | 0xBE | 0x7D | 0xFD | 0x3D | 0x1D | 0x5D | 0xDD | 0xFE | 0xDE
            | 0x1E | 0x5E | 0x3E | 0x7E => 4,
    ...
    fn lift_opcode(
        ...
        let imm =
            |bytes: &[u8], off: usize| -> u8 { if off < bytes.len() { bytes[off] } else { 0 } };
        ...
        match opcode {
            ...
            0x85 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreA(zp as u16)], 2))
            }
            0x95 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreA(zp as u16)], 2))
            ...
            0x86 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreX(zp as u16)], 2))
            }
            0x96 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreX(zp as u16)], 2))
            ...
            0x8E => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::StoreX(abs)], 3))
            }

            // LDY variants
            0xA0 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::LoadY(Operand::Immediate(val))], 2))
            }
            0xA4 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::LoadY(Operand::Zeropage(zp))], 2))
            }
            0xAC => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::LoadY(Operand::Absolute(abs))], 3))
            }
            0xB4 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::LoadY(Operand::ZeropageX(zp))], 2))
            }
            0xBC => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::LoadY(Operand::AbsoluteX(abs))], 3))
            }

            // STY variants
            0x84 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreY(zp as u16)], 2))
            }
            0x94 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::StoreY(zp as u16)], 2))
            ...
            0x8C => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::StoreY(abs)], 3))
            }

            // ADC variants (all 8 addressing modes)
            0x69 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::AddWithCarry(Operand::Immediate(val))], 2))
            }
            0x65 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::AddWithCarry(Operand::Zeropage(zp))], 2))
            }
            0x75 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::AddWithCarry(Operand::ZeropageX(zp))], 2))
            }
            0x6D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::AddWithCarry(Operand::Absolute(abs))], 3))
            }
            0x7D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::AddWithCarry(Operand::AbsoluteX(abs))], 3))
            }
            0x79 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::AddWithCarry(Operand::AbsoluteY(abs))], 3))
            }
            0x61 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::AddWithCarry(Operand::IndirectX(zp))], 2))
            }
            0x71 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::AddWithCarry(Operand::IndirectY(zp))], 2))
            }

            // SBC variants (all 8 addressing modes)
            0xE9 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::SubWithCarry(Operand::Immediate(val))], 2))
            }
            0xE5 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::SubWithCarry(Operand::Zeropage(zp))], 2))
            }
            0xF5 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::SubWithCarry(Operand::ZeropageX(zp))], 2))
            }
            0xED => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::SubWithCarry(Operand::Absolute(abs))], 3))
            }
            0xFD => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::SubWithCarry(Operand::AbsoluteX(abs))], 3))
            }
            0xF9 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::SubWithCarry(Operand::AbsoluteY(abs))], 3))
            }
            0xE1 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::SubWithCarry(Operand::IndirectX(zp))], 2))
            }
            0xF1 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::SubWithCarry(Operand::IndirectY(zp))], 2))
            }

            // AND variants (all 8 addressing modes)
            0x29 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::And(Operand::Immediate(val))], 2))
            }
            0x25 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::And(Operand::Zeropage(zp))], 2))
            }
            0x35 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::And(Operand::ZeropageX(zp))], 2))
            }
            0x2D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::And(Operand::Absolute(abs))], 3))
            }
            0x3D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::And(Operand::AbsoluteX(abs))], 3))
            }
            0x39 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::And(Operand::AbsoluteY(abs))], 3))
            }
            0x21 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::And(Operand::IndirectX(zp))], 2))
            }
            0x31 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::And(Operand::IndirectY(zp))], 2))
            }

            // ORA variants (all 8 addressing modes)
            0x09 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::Or(Operand::Immediate(val))], 2))
            }
            0x05 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Or(Operand::Zeropage(zp))], 2))
            }
            0x15 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Or(Operand::ZeropageX(zp))], 2))
            }
            0x0D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Or(Operand::Absolute(abs))], 3))
            }
            0x1D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Or(Operand::AbsoluteX(abs))], 3))
            }
            0x19 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Or(Operand::AbsoluteY(abs))], 3))
            }
            0x01 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Or(Operand::IndirectX(zp))], 2))
            }
            0x11 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Or(Operand::IndirectY(zp))], 2))
            }

            // EOR variants (all 8 addressing modes)
            0x49 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::Xor(Operand::Immediate(val))], 2))
            }
            0x45 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Xor(Operand::Zeropage(zp))], 2))
            }
            0x55 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Xor(Operand::ZeropageX(zp))], 2))
            }
            0x4D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Xor(Operand::Absolute(abs))], 3))
            }
            0x5D => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Xor(Operand::AbsoluteX(abs))], 3))
            }
            0x59 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Xor(Operand::AbsoluteY(abs))], 3))
            }
            0x41 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Xor(Operand::IndirectX(zp))], 2))
            }
            0x51 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Xor(Operand::IndirectY(zp))], 2))
            }

            // CMP variants (all 8 addressing modes)
            0xC9 => {
                let val = imm(bytes, offset + 1);
                Some((vec![IrOp::Compare(Operand::Immediate(val))], 2))
            }
            0xC5 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Compare(Operand::Zeropage(zp))], 2))
            }
            0xD5 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Compare(Operand::ZeropageX(zp))], 2))
            }
            0xCD => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Compare(Operand::Absolute(abs))], 3))
            }
            0xDD => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Compare(Operand::AbsoluteX(abs))], 3))
            }
            0xD9 => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::Compare(Operand::AbsoluteY(abs))], 3))
            }
            0xC1 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Compare(Operand::IndirectX(zp))], 2))
            }
            0xD1 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::Compare(Operand::IndirectY(zp))], 2))
            }
            ...
            0x00 => Some((
                ...
                1,
            )), // BRK — special: software interrupt
            ...
            0x10 => {
                ...
                Some((
                    vec![IrOp::Branch {
                        condition: BranchCondition::Pl,
                        target: instr_addr.wrapping_add_signed(offset as i16 + 2),
                    }],
                    2,
                ))
            }
            ...
            0x06 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::ShiftLeft(zp as u16)], 2))
            }
            0x16 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::ShiftLeft(zp as u16)], 2))
            ...
            0x46 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::ShiftRight(zp as u16)], 2))
            }
            0x56 => {
                let zp = imm(bytes, offset + 1);
                Some((vec![IrOp::ShiftRight(zp as u16)], 2))
            ...
            0x4E => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::ShiftRight(abs)], 3))
            }
            0x5E => {
                let abs = addr(bytes, offset + 1, 2);
                Some((vec![IrOp::ShiftRight(abs)], 3))
            ...
            _ => Some((vec![IrOp::Nop], 1)),
        }
    }

    /// Convert a block of IR ops into an IrBlock with proper labeling
    pub fn build_ir_block(&mut self, start_addr: u16, ops: Vec<IrOp>) -> IrBlock {
        IrBlock {
            start: start_addr,
            ops,
            terminator: None,
